...
thread_run(kz_func_t func, char* name, int priority, int stacksize, int argc, char* argv[])
{
    int i;
    kz_thread* thp;
    uint32* sp;
    ...
    *(--sp) = (uint32)thp; /* ER0 : argv[0] */
    ...
    /** put thread generated new to ready queue */
    current = thp;
    putcurrent();
...
static kz_thread_id_t
thread_recv(kz_msgbox_id_t id, int* sizep, char** pp)
{
    kz_msgbox* mboxp = &msgboxes[id];
...
call_functions(kz_syscall_type_t type, kz_syscall_param_t* p)
{
    ...
    case KZ_SYSCALL_TYPE_KMFREE:
        p->un.kmalloc.ret = thread_kmfree(p->un.kmfree.p);
...
void
kz_start(kz_func_t func, char* name, int priority, int stacksize, int argc, char* argv[])
{
    kzmem_init();
